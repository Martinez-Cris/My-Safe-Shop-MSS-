    ...
    return new Promise((resolve, reject) => {
      const uploadStream = cloudinary.uploader.upload_stream(
        { folder, resource_type: 'image',
          ...
          public_id: productName ? `${Date.now()}-${productName.toLowerCase().replace(/\s+/g, '-')}` : undefined,
        },
        (error, result: UploadApiResponse) => {
          if (error) reject(new BadRequestException(`Error al subir imagen: ${error.message}`));
          else resolve({ publicId: result.public_id, secureUrl: result.secure_url, width: result.width, height: result.height });
        },
      );
